...
- Class Email: [stat135-staff\@lists.berkeley.edu](mailto:stat135-staff@lists.berkeley.edu){.email}
...
Here we pre-define the `out` vector as a length 9 vector of `NA`s. When R goes through the loop, it can just replace theith value of `out` with `"even"` or `"odd"`.
...
**Question:** what is the value if `i`?
